 ...
 *    calculations involving tmp, ICR1 and TCNT1 are not truncated.
 *  - Ensure 16 bit registers accesses are atomic - run with interrupts
 *    disabled when accessing.
 ...
 *    experiment shows that overflow interrupt flag gets set whilst TCNT1 == 0,
 *    resulting in a phantom interrupt. Could just set to 1, but gets
 *    inaccurate at very short durations
...
TimerOne Timer1; // pre-instatiate
...
void TimerOne::pwm(char pin, int duty, long microseconds)
{
  ...
  setPwmDuty(pin, duty);
  // Lex - make sure the clock is running.  We don't want to restart the count,
  // in case we are starting the second WGM and the first one is in the middle o
  // of a cycle
  resume();
}
...
void TimerOne::attachInterrupt(void (*isr)(), long microseconds)
{
  ...
  // might be running with interrupts disabled (eg inside an ISR), so don't
